...
fn validate_register_request(req: &RegisterRequest) -> Result<(), AppError> {
    ...
    if req.username.len() < 3 {
        return Err(AppError::Validation(
            "Username minimal 3 karakter".to_string(),
        ));
    }
...
async fn test_register_validate_input() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Validasi dengan password pendek seharusnya gagal"
    );

    let err = result.unwrap_err();
    assert!(
        format!("{:?}", err).contains("password") || format!("{:?}", err).contains("Password"),
    ...
    assert!(
        result.is_err(),
        "Validasi dengan username pendek seharusnya gagal"
    );

    let err = result.unwrap_err();
    assert!(
        format!("{:?}", err).contains("username") || format!("{:?}", err).contains("Username"),
    ...
    assert!(
        result.is_ok(),
        "Validasi dengan input valid seharusnya berhasil"
    );
...
async fn test_login_password_verification() -> Result<()> {
    ...
    assert!(
        ...
        "Validasi password mock, kedua hasil bisa diterima"
    );
